...
_carts.post = function(data,callback){
  ...
  var menuId = typeof(data.payload.menuId) == 'object' && data.payload.menuId instanceof Array && data.payload.menuId.length > 0 ? data.payload.menuId : false;
  if(email && menuId){
      ...
      if(tokenIsValid){
        ...
        _data.read('menus','menus',function(err,data){
          if(!err && data){
            ...
            menuId.forEach(function(id){
              menuList.forEach(function(menu){
                if(id==menu.id){
                  selectedList.push(menu.id);
                }
              });
            });
            // Convert selected list from array to json
            var selectedListJson = {menuId : selectedList};
              ...
              if(!err && cartData){
                ...
                _data.delete('carts',email,function(err){
                  if(!err){
                    ...
                    _data.create('carts',email,selectedListJson,function(err){
                      if(!err){
                        callback(200);
                      } else {
          ...
          if(!err && cartData){
            _data.read('menus','menus',function(err,menuData){
              if(!err && menuData){
                ...
                cartData.menuId.forEach(function(cartId){
